      ...
      <div className="lv-container">
        <div className={cx("overflow-hidden border", theme.radius)} style={{ borderColor: "var(--lv-line)", background: "var(--lv-surface)" }}>
          <div className="grid gap-10 p-8 sm:p-12 lg:grid-cols-2 lg:gap-16">
            <Reveal>
              <h2 className={cx("text-[clamp(1.75rem,3.4vw,2.5rem)] font-semibold text-[var(--lv-ink)]", theme.headFont, theme.tracking)}>
                {contact.heading}
              </h2>
              <p className="mt-3 max-w-sm text-[15px] leading-relaxed text-[var(--lv-ink-muted)]">{contact.body}</p>
            </Reveal>
            <Reveal delay={0.1}>
              <dl className="grid gap-4 sm:grid-cols-2">
                {rows.map((r) => (
                  <div key={r.label} className="flex items-start gap-3">
                    ...
                    <div>
                      <dt className="text-[11.5px] uppercase tracking-wide text-[var(--lv-ink-muted)]">{r.label}</dt>
                      <dd className="mt-0.5 text-[14px] font-medium text-[var(--lv-ink)]">{r.value}</dd>
                    </div>
                  </div>
                ))}
              </dl>
