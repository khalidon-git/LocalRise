...
the relevant doc before changing a subtle system â€” most rules below exist because
something failed once.

## What this is
A marketing site for a digital agency serving local Indian businesses.
**Statically-exported Next.js 14 App Router** â€” no backend, no API routes, no database.

| | |
| --- | --- |
| Framework | **Next.js 14.2.15** (App Router) â€” **not 15** |
| React | **18.3.1** â€” not 19 |
| Language | TypeScript 5.6, `strict` |
| Styles | Tailwind 3.4 (tokens in `tailwind.config.ts`) |
| Animation | **Framer Motion 11** only â€” no GSAP |
| Output | `output: "export"` â†’ static `out/`, `trailingSlash: true`, images unoptimised |

## Hard invariants â€” do not violate
1. **No raw `<a href="/â€¦">` for internal links.** Use `SmartLink` / `Button href`. A raw anchor reloads the document and destroys the audio engine. (`docs/navigation.md`, `knowledge/bugs/001`)
...
4. **No Next 15 / React 19 idioms** â€” no async `params`, `useActionState`, or `use()`.
5. **Static export** â€” no server actions, route handlers, runtime env, or ISR. Read `localStorage`/`window` in effects, never during render. Param routes need `generateStaticParams`. Never commit `out/`.
6. **Content lives in `lib/content`** (barrel `@/lib/content`), never hard-coded. Keep coupled shapes in sync: `services`â‡„`serviceDetails` (shared `id`), `serviceDetails.faqPicks` (indices into `faqs`), `relatedPackageId` â†’ `packages[].id`.
7. **Honest content** â€” no fabricated testimonials, client logos, or metrics; concept sites stay labelled "Design Concept". A validator demanding a field is not licence to invent it. (`knowledge/decisions/004`)
8. **Zero external requests** â€” self-hosted fonts, inline SVG. Don't add Google Fonts, CDNs, or new runtime dependencies without a strong reason.
...
10. **Verify the artifact, not the status.** `npx tsc --noEmit` for type safety; `npm run build` for anything touching routes/metadata/config, and confirm `out/` is correct. Green â‰  working (`knowledge/bugs/005`).

## Delegate to specialised subagents (`.codex/agents/`)
Prefer routing focused work to the right specialist â€” it keeps context small, edits
...
| Motion â€” Framer / Tailwind keyframes / scroll / micro-interactions | **animation-specialist** |
...
**Default flow**: for a UI change â†’ build with the relevant specialist, then run
...
```
Deploy: push to `main` â†’ GitHub Action builds the `deploy` branch â†’ Redeploy in
